...
- **`jsont`** — schema, parser, validator, transformer, and row writer
- **`jsont-crypto`** — envelope encryption (ChaCha20-Poly1305, AES-256-GCM, ASCON-128a) with RSA and ECDH DEK wrapping
...
JsonT supports field-level encryption via **privacy markers** (`~`) on schema fields
and the `jsont-crypto` crate, which provides envelope encryption with per-stream DEK management.

**Algorithm support:**

| Version constant | Cipher | DEK wrapping |
|---|---|---|
| `CryptoContext::VERSION_CHACHA_PUBKEY` (`0x0010`) | ChaCha20-Poly1305 | RSA-OAEP-SHA256 |
| `CryptoContext::VERSION_CHACHA_ECDH` (`0x0011`) | ChaCha20-Poly1305 | ECDH P-256 + HKDF |
| `CryptoContext::VERSION_AES_PUBKEY` (`0x0008`) | AES-256-GCM | RSA-OAEP-SHA256 |
| `CryptoContext::VERSION_AES_ECDH` (`0x0009`) | AES-256-GCM | ECDH P-256 + HKDF |
| `CryptoContext::VERSION_ASCON_PUBKEY` (`0x0018`) | ASCON-128a | RSA-OAEP-SHA256 |
| `CryptoContext::VERSION_ASCON_ECDH` (`0x0019`) | ASCON-128a | ECDH P-256 + HKDF |

Each sensitive field gets its own independently generated random IV. The DEK is shared across
all fields in the stream and is written once (wrapped) in the `EncryptHeader` row.
...
`write_encrypted_stream` writes the `EncryptHeader` row followed by all data rows.
Sensitive fields are encrypted with a fresh random IV per field per row.

```rust
use std::{fs::File, io::{BufWriter, Write}};
use jsont::{
    write_encrypted_stream, EnvCryptoConfig, CryptoContext, SchemaKind,
    JsonTFieldBuilder, JsonTSchemaBuilder, JsonTRowBuilder, JsonTValue, ScalarType,
};
...
let rows = vec![
    JsonTRowBuilder::new()
        ...
        .push(JsonTValue::str("123-45-6789"))   // plaintext; encrypted on write
        .build(),
];

// EnvCryptoConfig reads PEM key material from environment variables at call time.
...
For ECDH instead of RSA, use `EcdhCryptoConfig`:

```rust
use jsont::EcdhCryptoConfig;

// Rust = party A: peer_pub = party B's DER public key; host_priv = party A's PEM private key
let peer_pub  = std::fs::read("ec_b_public.der")?;
let host_priv = std::fs::read_to_string("ec_a_private.pem")?;
let cfg = EcdhCryptoConfig::of_keys(peer_pub, &host_priv);
// Use exactly like EnvCryptoConfig above
```
...
use std::fs;
use jsont::{
    try_parse_encrypt_header, parse_field_payload,
    EnvCryptoConfig, JsonTRow, JsonTValue, Parseable, ValidationPipeline,
};

let content = fs::read_to_string("output.jsont")?;
let all_rows = Vec::<JsonTRow>::parse(&content)?;

// Row 0 is always the EncryptHeader.
let ctx = try_parse_encrypt_header(&all_rows[0])
    .expect("first row must be an EncryptHeader");

let cfg = EnvCryptoConfig::new("JSONT_PUBLIC_KEY", "JSONT_PRIVATE_KEY");

// Open a session — unwraps the DEK once; the session holds the plaintext DEK.
// CipherSession zeros the DEK on drop.
let pipeline_session = cfg.open_session(&ctx)?;

let pipeline = ValidationPipeline::builder(schema)
    .without_console()
    .with_cipher_session(pipeline_session)   // session moves into the pipeline
    .build()?;

let clean_rows = pipeline.validate_rows(all_rows[1..].to_vec());
pipeline.finish()?;

// After validation, sensitive fields are JsonTValue::Encrypted(envelope_bytes).
// Open a second session for manual field-level decryption.
let verify_session = cfg.open_session(&ctx)?;

match clean_rows[0].get(1) {
    Some(JsonTValue::Encrypted(envelope)) => {
        let (iv, _digest, enc_content) = parse_field_payload(envelope, "ssn")?;
        let plaintext = verify_session.decrypt_field(&iv, &enc_content)?;
        println!("ssn = {}", String::from_utf8(plaintext)?);
    }
...
An encrypted stream always starts with one `EncryptHeader` row, then normal data rows:

```
{"ENCRYPTED_HEADER", <version:u16>, <length:u32>, "<base64:enc_dek>"}
{"Alice", "<base64:field_payload>"}
{"Bob",   "<base64:field_payload>"}
```

Each `field_payload` is a binary blob (base64-encoded on the wire):

```
[2B: len_iv][4B: len_digest_bits][iv bytes][sha256 digest][ciphertext + auth tag]
```

The DEK is shared per stream; IV is fresh per field invocation — `(DEK, IV)` pairs are never reused.
...
match err {
    ...
    JsonTError::Transform(TransformError::DecryptFailed { field, reason }) => {
        // Decrypt operation failed — no CipherSession supplied, or session.decrypt_field() error
    }
